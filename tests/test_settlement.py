...
async def test_settlement_computes_correct_amount(db):
    """settlement_amount = (inr_amount - fee_inr) / fx_rate"""
    ...
    fixed_rate = Decimal("83.5")
    with patch("services.fx_service.convert", new=AsyncMock(return_value=(fixed_rate, Decimal("1000.0000")))):
        tx = await process_settlement(tx.id, db)

    assert tx.status == TransactionStatus.settled
    assert tx.fx_rate == fixed_rate
    # fee_inr = 83500 * 1.5% = 1252.50; fee_in_settlement = 1252.50 / 83.5 = 15.0000
    # net = 1000 - 15 = 985.0000
...
async def test_settlement_creates_reconciliation_log(db):
    ...
    tx = await simulate_inr_collection(tx.id, db)
    fixed_rate = Decimal("83.5")
    with patch("services.fx_service.convert", new=AsyncMock(return_value=(fixed_rate, Decimal("598.8024")))):
        tx = await process_settlement(tx.id, db)
